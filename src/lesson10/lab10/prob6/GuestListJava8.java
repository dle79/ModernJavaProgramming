...
public class GuestListJava8 {
	...
	public void printListOfExpectedFemaleGuests(List<Guest> invited) {
		// implement
		String result = invited
				.stream()
				.filter(s -> PredicateWithException.unchecked(
						(Guest g) -> checkLegal(g)).test(s))
				.filter(s -> s.getGender() == Gender.F && s.isPlanningToCome())
